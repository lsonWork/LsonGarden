            ...
            <table class="table">
                <thead class="thead-dark">
                    <tr style="text-align: center">
                        ...
                        <th scope="col">IDPlant</th>
                        <th scope="col"></th>
                        <th scope="col">Status</th>
                            <% if (((String)session.getAttribute("roleToRedirrect")).equalsIgnoreCase("Admin")) { %>
                        <th scope="col"></th>
                        <th scope="col"></th>
                            <% 
                                    }
                ...
                <tbody>
                    <c:forEach items="${listShowOrders}" var="s">
                        <tr style="text-align: center">
                            <td>${s.getName()}</td>
                            <td>${s.getQuantity()}</td>
                            <td>${s.getPrice()}</td>
                            ...
                            <td>
                                <form action="ShowOrdersServlet" method="post">
                                    <input type="checkbox" name="checkStatus" ${s.getStatus() == 0 ? '' : 'checked'}>    
                                    <input type="hidden" name="idOrderToStatus" value="${s.getId()}">
                                    <input type="hidden" name="checkToShowSet" value="0">
                                    <input type="submit" name="submitStatus" value="Set">
                                </form>
                            </td>
                            <td>${s.getStatus() == 0 ? 'Processing' : 'Approved'}</td>
                            <c:if test="${not empty s.getSeller()}">
                                <% if (((String)session.getAttribute("roleToRedirrect")).equalsIgnoreCase("Admin")) { %>
                                <td>${s.getSeller()}</td>    
                                <% 
                                }
                            ...
                            <td>
                                <form action="DeleteOrderServlet" method="post">
                                    ...
                                    <input type="hidden" name="deleteOrder" value="${s.getId()}">
                                </form>
                            </td>
                            <% 
                            }
